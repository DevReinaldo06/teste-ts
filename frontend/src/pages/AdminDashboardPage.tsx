...
// import { Firestore } from 'firebase/firestore'; // REMOVIDO

import { AppPage, Card, User, AdminView } from '../types/gameTypes';
// Atualização de importações
import { addCard, updateCard, deleteCard, updateUserProfile, deleteUser } from '../services/apiService'; // NOVO
...
  // db: Firestore | null; // REMOVIDO
  cards: Card[];
  users: User[];
  setCurrentPage: (page: AppPage) => void;
  setAppMessage: (msg: string | null) => void;
  // cardCollectionPath: string; // REMOVIDO
  // userCollectionPath: string; // REMOVIDO
  refreshData: () => Promise<void>; // NOVO: Função para recarregar dados do App.tsx
}> = ({ cards, users, setCurrentPage, setAppMessage, refreshData }) => {
...
    try {
      await addCard(card); // Função atualizada para usar a API
      await refreshData(); // Recarrega a lista de cards
...
    try {
      await updateCard(card); // Função atualizada para usar a API
      await refreshData(); // Recarrega a lista de cards
...
    try {
      await deleteCard(id); // Função atualizada para usar a API
      await refreshData(); // Recarrega a lista de cards
...
    try {
      await updateUserProfile(user); // Função atualizada para usar a API
      await refreshData(); // Recarrega a lista de usuários
...
    try {
      await deleteUser(id); // Função atualizada para usar a API
      await refreshData(); // Recarrega a lista de usuários
