...
import { updateUserProfile } from '../services/apiService'; // NOVO
...
}> = ({ currentUser, cards, setCurrentPage, setCurrentUser, setAppMessage }) => {
...
  // Garante que o estado interno do email/senha seja populado
...
    e.preventDefault();
    if (!currentUser || !newEmail || !newPassword) return;

    try {
      // Cria um objeto de usuário atualizado para enviar à API
      const userToUpdate: User = { ...currentUser, email: newEmail, password: newPassword }; 
      
      await updateUserProfile(userToUpdate); // Usa a função atualizada para usar a API
      
...
            <button
              onClick={() => { 
                // Garante que o estado de edição use os dados atuais antes de abrir o modal
